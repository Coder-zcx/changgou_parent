...
public class UserServiceImpl implements UserService {
    ...
    public int updateUserPoints(Task task) {
        ...
        redisTemplate.boundValueOps(task.getId()).set("exit", 1, TimeUnit.MINUTES);

        int count = userMapper.updateUserPoint(userName, point);
        if (count <= 0) {
            return count;
        }
        ...
        count = pointLogMapper.insertSelective(log);
        if (count <= 0) {
            return count;
        }
